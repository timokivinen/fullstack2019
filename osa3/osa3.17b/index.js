// --------------------------------------------------------------------------
// --- Tehtävä:         3.17b: puhelinluettelo ja tietokanta, step4
// --- Pvm:             24.8.2019 Timo Kivinen
// ---                  BACKEND, FrontEnd = 3.17f
// --- Aikaa käytetty:  5 h
// --------------------------------------------------------------------------
...
app.post('/api/persons', (req, res, next) => {
  ...
  const errorHandler = (error, req, res, next) => {
    ...
    if (error === 'name is mandatory') {
	  console.log(`ERRORMSG MAdaTORY`)		
      return res.status(400).send({ error: 'name is mandatory' })
    }
    if (error === 'number is mandatory') {
		console.log(`ERRORMSG MAdaTORY`)
      return res.status(400).send({ error: 'number is mandatory' })
    }


	console.log(`ERRORMSG: ${error}. ERR.NAME: ${error.name} ERR.KIND: ${error.kind} \n`)
	// return res.status(405).send({ error: 'number is mandatory' })	  
    next(error)
  }
